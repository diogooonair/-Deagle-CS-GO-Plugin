...
public Action OnRoundStart(Event event, const char[] name, bool dontBroadcast)
{
	if(g_voteyes > g_voteno)
	{
		...
		g_voteyes = 0;
		g_voteno = 0;
		PrintToChatAll("As proximas 3 rondas vao ser jogadas sem deagle!!!");
    }
...
public int AskIfWantToRestrictDeagle(int client)
{
   if(!g_alreadythismap)
   {
      ...
      menu.SetTitle("Queres restringir a deagle por 3 rondas?");
      menu.AddItem("yes", "Sim");
      menu.AddItem("no", "Nao");
   ...
   {
		PrintToChat(client, "Este comando so pode ser usado uma vez por mapa");
   }
}
